...
def item_sim(args):
    fp0 = f"../embeddings/ml-1m_{args.pooling}_plain.npy" 
    fp1 = f"../embeddings/ml-1m_{args.pooling}.npy" 
    fp2 = "../data/ml-1m/saved_embed/lightgcn_item_emb.npy"
    # fp3 = "../data/ml-1m/saved_embed/lightgcn_item_emb_ssl.npy"
    fp3 = f"../embeddings/ml-1m_{args.pooling}_ssl.npy" 
    colla_id2item = json.load(open("../data/ml-1m/saved_embed/lightgcn_id2item.json", "r"))
    ...
    fp_indice =os.path.join(args.embed_dir, '_'.join(["ml-1m", args.embed_type, "indice"])+".npy")
...
def item_sim_ssl(args):
    fp1 = f"../embeddings/ml-1m_{args.pooling}_ssl.npy" 
    fp2 = "../data/ml-1m/saved_embed/lightgcn_item_emb_ssl.npy"
    colla_id2item = json.load(open("../data/ml-1m/saved_embed/lightgcn_id2item.json", "r"))
    ...
    fp_indice =os.path.join(args.embed_dir, '_'.join(["ml-1m", args.embed_type, "indice"])+"_ssl.npy")
    np.save(fp_indice, sorted_indice)
    print("Saved.")
    
def user_sim(args):
    fp = "../data/ml-1m/saved_embed/lightgcn_user_emb.npy"
    embed = np.load(fp)
    print(embed.shape)
    ...
    fp_indice =os.path.join(args.embed_dir, '_'.join(["ml-1m", "user", "indice"])+".npy")
    np.save(fp_indice, sorted_indice)
    print("Saved.")
